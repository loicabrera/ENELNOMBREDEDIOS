  ...
  return (
    <aside className="h-screen w-64 bg-white border-r shadow-sm fixed top-0 left-0 z-50">
      ...
      <nav className="p-4 flex flex-col gap-2">
        <NavItem icon={<Home size={20} />} label="Inicio" onClick={() => navigate('/dashboardadmin')} />
        <NavItem icon={<Users size={20} />} label="Proveedores" onClick={() => navigate('/dashboardadmin/proveedores')} />
...
function NavItem({ icon, label, onClick }) {
  return (
    <button onClick={onClick} className="flex items-center gap-3 px-4 py-2 rounded-md text-gray-700 hover:bg-gray-100 transition-all text-sm font-medium">
      {icon}
      {label}
    </button>
  );
